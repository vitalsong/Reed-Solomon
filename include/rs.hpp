...
#define RS_MEM_SIZE(MSG, ECC) ((MSG_CNT) * (MSG) + (POLY_CNT) * (ECC) * 2)
...
public:
     ...
     * @param mem - extern mem for embedded systems used (use RS_MEM_SIZE macro for detect size) */
    ReedSolomon(uint8_t msg_length, uint8_t ecc_length, uint8_t* ext_mem = NULL);
    ~ReedSolomon();
...
//---------------------------------------------------------------------------------------------------
ReedSolomon::ReedSolomon(uint8_t _msg_length, uint8_t _ecc_length, uint8_t *ext_mem) :
    ...
    ecc_length(_ecc_length)
{
    if (ext_mem != NULL) {
        ...
        memory = new uint8_t[RS_MEM_SIZE(msg_length, ecc_length)];
        int_mem = true;
    }
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::EncodeBlock(const void *src, void *dst)
{
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::Encode(const void *src, void *dst)
{
    uint8_t* dst_ptr = (uint8_t*) dst;
...
//---------------------------------------------------------------------------------------------------
int ReedSolomon::DecodeBlock(const void *src, const void *ecc, void *dst, uint8_t *erase_pos, size_t erase_count)
{
    assert(msg_length + ecc_length < 256);
...
//---------------------------------------------------------------------------------------------------
int ReedSolomon::Decode(const void *src, void *dst, uint8_t *erase_pos, size_t erase_count)
{
    const uint8_t *src_ptr = (const uint8_t*) src;
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::GeneratorPoly()
{
    Poly *gen = polynoms + ID_GENERATOR;
    gen->at(0) = 1;
    gen->length = 1;
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::CalcSyndromes(const Poly *msg)
{
    Poly *synd = &polynoms[ID_SYNDROMES];
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::FindErrataLocator(const Poly *epos)
{
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::FindErrorEvaluator(const Poly *synd, const Poly *errata_loc, Poly *dst, uint8_t ecclen)
{
    Poly *mulp = &polynoms[ID_TPOLY1];
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::CorrectErrata(const Poly *synd, const Poly *err_pos, const Poly *msg_in)
{
...
//---------------------------------------------------------------------------------------------------
bool ReedSolomon::FindErrorLocator(const Poly *synd, Poly *erase_loc, size_t erase_count)
{
...
//---------------------------------------------------------------------------------------------------
bool ReedSolomon::FindErrors(const Poly *error_loc, size_t msg_in_size)
{
    Poly *err = &polynoms[ID_ERRORS];
...
//---------------------------------------------------------------------------------------------------
void ReedSolomon::CalcForneySyndromes(const Poly *synd, const Poly *erasures_pos, size_t msg_in_size)
{
